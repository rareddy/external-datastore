...
IMAGE ?= docker.io/teiid/external-datastore
#TAG ?= $(shell git describe --tags --always)
TAG ?= latest
PULL ?= IfNotPresent

build: ## Builds the starter pack
	go build -i github.com/teiid/external-datastore/cmd/servicebroker
...
linux: ## Builds a Linux executable
	...
	go build -o servicebroker-linux --ldflags="-s" github.com/teiid/external-datastore/cmd/servicebroker
...
deploy-openshift: image ## Deploys image to openshift
	oc new-project external-datastore
